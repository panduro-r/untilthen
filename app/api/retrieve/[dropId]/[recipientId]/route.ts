...
export async function GET(
  ...
  // The atomic burn below requires released_at, which for multi-sig is normally stamped by the daily
  // cron. Catch the DB up from the chain first so a recipient isn't blocked waiting. Best-effort: any
  // failure falls through to the same atomic burn + uniform 410, so it never leaks or weakens it.
  try {
    await syncMultisigRelease(dropId)
  } catch (e) {
    console.error("[retrieve] multisig release sync failed:", e)
  }
