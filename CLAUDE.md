...
- Open: SRI/reproducible build; origin binding on wallet signatures (`SECURITY.md` gap 6). **Storage-lifetime work is PARKED by decision:** the 48h blob cap is a temporary Shelby-team cap (was 24h, expected to rise, no ETA), and this is a testing-phase project with no real users — so no arm-date guardrail, no check-in renewal, and no user-facing "storage lasts 48h" copy yet. When revisited, do the **don't-burn-the-link-on-a-dead-blob** fix first (`lib/decrypt.ts` burns the one-time link *before* downloading the blob → an expired blob permanently loses the file).
...
**Migrations** run `0001`–`0011`. `0001`–`0005` are bare `CREATE` (not idempotent) — apply only the new file when adding one: `node scripts/migrate.mjs 00NN_name.sql`. Live function definitions have DRIFTED from the early files, so dump the current def (`pg_get_functiondef`) before recreating one rather than copying from `0002`. Security-relevant DB state (keep it this way): all 7 public tables have RLS + a policy, all 5 functions are SECURITY INVOKER with `search_path=''` pinned, and `anon`/`authenticated` hold **zero** table privileges (`0011`). Never mark one of those functions SECURITY DEFINER — RLS is what makes the public anon key harmless.
...
**Two kinds of wallet signature — do NOT mix them.** `lib/aptos.signMessageFull` is shared, but its callers split into two classes:
- **Key material** (`ownerCopyMessage`, `signerEncMessage`): the signature is fed to `deriveWalletWrapKey` / `deriveSignerEncKeypair` **client-side only** and never sent to the server. These messages and the `signMessage` payload shape must stay **byte-stable forever** — changing either changes the derived key and breaks reset/recovery on every existing safe and every signer's ECIES keypair.
- **Server-verified auth** (`siwaMessage`, `ownerAuthMessage`, `registerMessage`, `signerRegisterMessage`): checked by `verifyAptosSignedMessage`, never used to derive a key. Safe to change. `ownerAuthMessage` now embeds an `Issued:` timestamp and expires after `OWNER_AUTH_MAX_AGE_MS` (10m) — without it, one captured signature was a permanent capability to overwrite a safe's tlock ciphertext via `reset`.

Known gap: no origin binding on the server-verified ones (we don't request the wallet's `application` field), so a phishing dapp can harvest an accepted signature. Fixing it needs a SEPARATE auth-only signing bridge — adding `application` to the shared bridge would break the key-material class above. See `SECURITY.md` gap 6.
