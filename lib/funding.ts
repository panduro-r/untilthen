...
function aptos(network?: AppNetwork): Aptos {
  ...
  const clientConfig = aptosClientConfigFor(net) // right key per network (Shelby on shelbynet, Aptos else)
...
async function shelbyClient(net?: AppNetwork) {
  ...
  return new ShelbyClient({
    network,
    ...(shelbyKey ? { apiKey: shelbyKey } : {}),
    aptos: { network, ...(aptosCfg ? { clientConfig: aptosCfg } : {}) },
  })
}
